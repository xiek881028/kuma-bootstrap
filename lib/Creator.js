...
module.exports = class Creator extends EventEmitter {
  ...
  async create({ dev, ...cliOptions } = {}) {
    ...
    deps.forEach(dep => {
      ...
      // 开发环境下记录插件包，先进行 lint 尝试
      if (dev && this.plugin[dep].link) {
        linkPkg.push(dep);
      } else {
    ...
    if (dev) {
      ...
      log();
      for (let i = 0; i < linkPkg.length; i++) {
        try {
          await pm.link([linkPkg[i]]);
        } catch (error) {
          warn(`插件包 ${linkPkg[i]} 链接失败`);
        }
      }
    }
