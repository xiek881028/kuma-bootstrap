...
class GeneratorAPI {
  ...
  extendRequire(dir, imported) {
    ...
    const rootVar = root
      ...
      .filter(path => path.parentPath.parentPath.value.type === 'Program' && path.value.declarations[0].init?.callee?.name === 'require');
    const impBody = imp.nodes()[0].program.body[0];
    const impName = impBody.declarations[0].id.name;
    const has = rootVar.__paths.findIndex(path => path.value.declarations[0].id.name === impName);
    if (has !== -1) {
      this.logger.error(`${impName} 在 ${path.relative(this.resolve('.'), dir)} 内已存在同名变量，写入失败`);
      return false;
    } else {
      ...
      const tempid = nanoid();
      if (index === -1) {// 结果为导向 有改进性，有进步的方向 前端创造的价值属于事业群还是属于基础研发（昌金） 数据不可信优先解决，量化标准，队列区分（容器云）（出规则）
  ...
  get useAppSet() {
    ...
    this.useAppArg(this.useAppNode).map((node, index) => nodeMap.set(node.name, index));
    return nodeMap;
  }

  useAppArg(context) {
    const j = this.jscodeshift;
    let arg = (context || this.useAppNode)
  ...
  get useAppNode() {
    ...
    return root
      ...
      .filter(path => path.value.object.name === 'app' && path.value.property.name === 'use')
  ...
  extendAppUse(useStr, ops = {}) {
    ...
    if (rootVar.length) {
      ...
      if (prev > next) {
        return this.logger.error(`befoer需要在${next}位之前，after需要在${prev}位之后，无法找到合适的位置插入app.use`);
      } else {
        // 原则上在可以插入的位置中挑选最靠后的位置插入
        this.useAppArg(rootVar)
          .splice(next, 0, j.identifier(impBody.declarations[0].id.name));
      }
      rootVar.insertBefore(impBody);
      fs.writeFileSync(this.resolve('app.js'), rootVar.toSource());
    } else {
      this.logger.error('app插件加入失败。未找到app.use，请检查是否更换了变量名或更换了写法。');
    }
  }
