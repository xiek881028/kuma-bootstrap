...
class PackageManager {
  constructor({ context, forcePackageManager, origin } = {}) {
    ...
    if (!SUPPORTED_PACKAGE_MANAGERS.includes(this.bin)) {
      log();
      warn(`您选择的安装工具没有官方支持，将使用npm进行安装。`);
  ...
  async link(arr = []) {
    await pMap(
      arr,
      async pkg => await executeCommand(this.bin, ['link', pkg], this.context),
      { concurrency: 4 }
    );
  }
  ...
  async getRegistry(scope) {
    ...
    const registry =
      args.registry ??
      registries[this.origin] ??
      this.origin ??
      (await execa(this.bin, ['config', 'get', 'registry'])).stdout;
